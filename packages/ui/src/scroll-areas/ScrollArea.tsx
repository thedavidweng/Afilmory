...
}) => <ScrollAreaBase.Corner {...rest} ref={forwardedRef} className={clsx('bg-accent', className)} />
...
const Root = ({
  ...
  <ScrollAreaBase.Root {...rest} scrollHideDelay={0} ref={forwardedRef} className={clsxm('overflow-hidden', className)}>
    {children}
    <Corner />
  </ScrollAreaBase.Root>
)
    ...
    <ScrollElementContext value={viewportRef}>
      <Root className={rootClassName}>
        <Viewport
          ...
          ref={setViewportRef}
          className={clsxm(flex ? '[&>div]:!flex [&>div]:!flex-col' : '', viewportClassName)}
          mask={mask}
          asChild={asChild}
          onScroll={onScroll}
          focusable={focusable}
